...
const CourseDetails = () => {
  ...
  const toggleSection = (index) => {
    setOpenSections((prev) => ({
      ...prev,
      [index]: !prev[index], // Toggle the state for the current section
    }))
  }
      ...
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left '>
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          ...
          <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
            ...
            <div className='flex'>{[...Array(5)].map((_, i) => (<img key={i} src={i < Math.floor(calculateRating(courseData)) ? assets.star : assets.star_blank} alt='' className='w-3.5 h-3.5' />))}</div>
            <p className='text-blue-600'>
              ({courseData.courseRatings.length} {courseData.courseRatings.length > 1 ? 'ratings' : 'rating'})
            </p>
            <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? 'students' : 'student'}</p>
          </div>
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>Scalenex</span></p>
                ...
                <div className='border border-gray-300 bg-white mb-2 rounded' key={index}>
                  <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={() => toggleSection(index)}>
                    <div className='flex items-center gap-2'>
                      <img
                        className={`transform transition-all duration-300 ${openSections[index] ? 'rotate-180' : ''}`}
          ...
          <div className='p-5'>
            ...
            <div className='pt-6'>
              ...
              <ul className='ml-4 pt-2 text-sm md:text-default list-disc text-gray-500'>
                <li>Lifetime access with free updates included.</li>
                <li>Comprehensive, step-by-step project guidance with hands-on experience.</li>
                <li>Access to downloadable resources and the source code.</li>
                <li>Quizzes to evaluate your knowledge and progress.</li>
